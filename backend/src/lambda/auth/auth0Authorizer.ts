...
const jwksUrl = 'https://dev-344f6yvspaqzcygv.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const publicKey: string = await getSigningKeys(res.data.keys,kid);
...
async function getSigningKeys(keys, kid) {
